---
title: "How to change the TDP on the GPD MicroPC 2"
type: source
subtype: kb-article
slug: gpd-micropc-2-tdp-change
brand: gpd
product: gpd-micropc-2
source_url: "https://gpdstore.net/kb/gpd-micropc-2-support-hub/kb-article/how-to-change-the-tdp-on-the-gpd-micropc-2/"
published: 2025-08-20
created: 2026-04-22
updated: 2026-04-22
tags: [kb-article, gpd, gpd-micropc-2]
---

Changing the TDP on a device allows you to increase or decrease the power usage, and in turn the performance of a device. This can be useful for example if you need more performance for higher demanding tasks, or low performance when reading an eBook. this step by step guide will show how to change the TDP on the [GPD MicroPC 2](https://gpdstore.net/laptops/gpd-micropc-2/).

## Accessing the BIOS

Switch on the [GPD MicroPC 2](https://gpdstore.net/laptops/gpd-micropc-2/) and immediately and repeatedly tap the **Backspace** key on the keyboard. Keep doing this until the BIOS menu is shown. If it boots to Windows, repeat the process again.

GPD MicroPC 2 Keyboard

## Changing the TDP

Using the **Up & Down** arrow keyboard keys, select **OEM System Configuration** and press the **Enter** key.

GPD MicroPC 2 BIOS Main Page

On the **OEM System Configuration** menu, navigate to **Configurable TDP** and press **Enter**

GPD MicroPC 2 OEM System Configuration

A menu will display with the available TDP settings (this may vary depending on the N250 or N300 model).

GPD MicroPC 2 TDP Selection

A lower Wattage number will consume less power and in return lower performance, while a higher number will use more power and in return higher performance. By default the TDP is set to 10W. Using the **Up & Down** keyboard keys, choose your TDP and press the **Enter** key to select it.

Press the **ESC** keyboard key once to return to the main menu and then use the **Left & Right** keyboard keys to change the menu to **Save & Exit**. Choose the **Save Changes and Exit** option.

GPD MicroPC 2 Save & Exit

Your [GPD MicroPC 2](https://gpdstore.net/laptops/gpd-micropc-2/) will now reboot with the newly chosen TDP and once booted into Windows you can continue to use your device.

## GPD MicroPC 2 UMPC

[Shop now](https://gpdstore.net/laptops/gpd-micropc-2/)
