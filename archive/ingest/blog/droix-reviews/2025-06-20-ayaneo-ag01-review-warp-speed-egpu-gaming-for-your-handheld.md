---
title: "AYANEO AG01 Review: WARP SPEED eGPU Gaming for Your Handheld"
source_url: "https://droix.net/blogs/ayaneo-ag01-review/"
source_site: droix.net
type: review-blog
date: 2025-06-20
---

**Captain’s Log, Stardate 25149.1.** Our mission: to conduct a thorough systems evaluation of a newly encountered artifact, the [AYANEO AG01 Starship graphics dock](https://droix.net/product/ayaneo-ag01/). We will assess its operational performance, focusing on its AMD Radeon graphics core, and determine in our AYANEO AG01 review if this device truly offers a quantum leap for gaming experiences.

Contents

[AYANEO AG01 Starship Review Video](#ayaneo-ag01-starship-review-video)

[AYANEO AG01 Starship Overview](#ayaneo-ag01-starship-overview)

[AYANEO AG01 Starship Technical Specifications](#ayaneo-ag01-starship-technical-specifications)

[Benchmarks](#benchmarks)

[3DMARK](#3dmark)

[Forza Horizon 5](#forza-horizon-5)

[Shadow of the Tomb Raider](#shadow-of-the-tomb-raider)

[Cyberpunk 2077](#cyberpunk-2077)

[Game Performance](#game-performance)

[Clair Obscure Expedition 33](#clair-obscure-expedition-33)

[Forza Horizon 5](#forza-horizon-5)

[No Mans Sky](#no-mans-sky)

[Indiana Jones and the Great Circle](#indiana-jones-and-the-great-circle)

[The Elder Scrolls IV: Oblivion Remastered](#the-elder-scrolls-iv-oblivion-remastered)

[Goat Simulator 3](#goat-simulator-3)

[DOOM Dark Ages](#doom-dark-ages)

[Final Thoughts](#final-thoughts)

## AYANEO AG01 Starship Review Video

<https://youtu.be/jIyTNBh9sgI>

## AYANEO AG01 Starship Overview

Lets get this AYANEO AG01 review started with an overview of the [eGPU](https://droix.net/product-category/egpu/). The [AYANEO AG01 Starship graphics dock](https://droix.net/product/ayaneo-ag01/) measures around 9.4 x 4.6 x 2.0 inches (24.0 x 11.8 x 5.1 cm) and weighs around 960g (2.11 lbs).

AYANEO AG01 Starship Top View

We couldn’t make this review without paying some special attention to the design. This is easily the best design for an eGPU docking station we have seen. Compared to the GPD and ONEXPLAYER designs, it’s boldly gone where no eGPU has gone before.

AYANEO AG01 Starship in Nebula Red colour scheme



AYANEO AG01 Starship Front View



AYANEO AG01 Starship Fan Exhaust

This is the Nebula Red model, and its contrasting red and black colours with silver text really stand out. The fan intakes look like observation windows on the side. And the fan exhaust, styled like twin engine nacelles, just screams for some RGB lighting which is sadly missing.

AYANEO AG01 Starship Power Dial and LED control butttons

The top features a pressable dial which controls the power and the performance profile, allowing you to divert power. You can also control the RGB lighting around this command dial with the two buttons beside it, which switch it off/on and cycle through various stellar patterns.

On the starboard side is where you can find all the connectivity. There are two DisplayPorts, two HDMI ports, an [OCuLink](https://droix.net/product-attribute/i-o-video/1x-oculink/) port, a USB-C port and the power port. Followed by a gigabit ethernet and USB-A port.

AYANEO AG01 ports

On the hull, you can find a stowaway compartment – no smuggling dodgy goods here, just a handy space for an M.2 2280 PCIe 3 SSD.

AYANEO AG01 Starship SSD Compartment

And a brief look at the included 330W power supply. It measures around 4.72 x 3.7 x 1.1 inches (12 x 9.5 x 3 cm) and weighs around 667g (1.4 lbs). It comes with the plug adaptor for your country or you can plug straight into the charger with a kettle lead. And included in the box are USB 4 and a branded OCuLink cable, so you have everything to get up and running straight away.

AYANEO AG01 330W Power Supply



AYANEO AG01 USB4 and OCuLink cables included

And included in the box are USB 4 and a branded OCuLink cable, so you have everything to get up and running straight away.

## AYANEO AG01 Starship Technical Specifications

For the AYANEO AG01 review, let’s take a peek under the hood, or should we say, into the engine room of this [AYANEO AG01 Starship graphics dock](https://droix.net/product/ayaneo-ag01/) star-cruiser.

|  |  |
| --- | --- |
| **GPU** | [AMD](https://droix.net/product-attribute/graphics-gpu-brand/amd/) [Radeon RX 7600M XT](https://droix.net/product-attribute/graphics-gpu-model/radeon-rx-7600m-xt/), 2,300MHz, 32 CU |
| **RAM** | [8GB](https://droix.net/product-attribute/graphics-gpu-memory/8gb/) [GDDR6](https://droix.net/product-attribute/graphics-gpu-memory-type/gddr6/) |
| **TGP** | 100W/120W via dial switch |
| **VIDEO** | [2x DisplayPort 2.0](https://droix.net/product-attribute/i-o-video/2x-displayport-4k60hz/), 80Gbps, 4K@120Hz / 8K@60Hz [2x HDMI 2.1](https://droix.net/product-attribute/i-o-video/2x-hdmi-4k60hz/), 48Gbps, 4K@120Hz / 8K@60Hz |
| **DATA** | [1x OCuLink](https://droix.net/product-attribute/i-o-video/1x-oculink/), PCIe 4.0×4, 64Gbps [1x USB4](https://droix.net/product-attribute/i-o-video/1x-via-usb-4-0-type-c-40gb-s/), 40Gbps, Supports 100W PD Reverse Power Supply [1x USB 3.2 Gen 2](https://droix.net/product-attribute/i-o-usb/1x-usb-type-a-3-2-gen-2/), 10Gbps (while USB4 connected) [1x Gigabit Ethernet](https://droix.net/product-attribute/ethernet/1x-rj45-port-1000mbps/) (while USB4 connected) [1x M.2 2280](https://droix.net/product-attribute/storage-expansion/1x-m-2-ssd-2280-slot/) PCIe 3.0 SSD (no SSD included) [1x SD card reader](https://droix.net/product-attribute/storage-expansion/1x-sd-card-slot/) 4.0, 300MB/s (while USB4 connected) |
| **SIZE** | 9.4 x 4.6 x 2.0 inches (24.0 x 11.8 x 5.1 cm) |
| **WEIGHT** | 960g (2.11 lbs) |

While performing our shakedown cruise we also checked the fan noise and temperatures. We got a high of 65 dB and 72 °C.

AYANEO AG01 Starship thermals

## Benchmarks

As part of our [AYANEO](https://droix.net/product-attribute/brands/ayaneo/) AG01 review we ran some benchmarks to compare the performance while using OCuLink with the [AYANEO 3](https://droix.net/product/ayaneo-3/), with the internal display, and a 4K monitor for external display.

AYANEO AG01 Starship with AYANEO 3

### 3DMARK

On 3DMARK we ran the Time Spy, Night Raid and Fire Strike benchmarks. These scores are impressive… most impressive! With results around 9,600 on Time Spy, over 50,000 on Night Raid and around 23,00 on Fire Strike. There is a little variation between internal and external displays, but not as much as we have seen in the past compared with other setups.

AYANEO AG01 3DMARK BENCHMARK COMPARISON

### Forza Horizon 5

For Forza Horizon 5  we are running on the Very Low graphics settings with no FSR enabled. At 4K we see a smooth 107 frames per second. As we drop down the resolutions to 720P we see 190 frames per second. The internal display also does very well, as far as an additional 26 frames per second at 720P!

AYANEO AG01 FORZA HORIZON 5 BENCHMARK COMPARISON

### Shadow of the Tomb Raider

For Shadow of the Tomb Raider we are running on the Low graphics settings with no FSR enabled.  At 4K we get a slightly above average 91 frames per second. There is a nice jump in performance at 1440P and levels out at 1080P and 720P. On the internal display we lose a few frames but get the same leveling out.

AYANEO AG01 SHADOW OF THE TOMB RAIDER BENCHMARK COMPARISON

### Cyberpunk 2077

For Cyberpunk we are running on the Low graphics settings with no FSR enabled. This demanding game only gets around 41 frames per second at 4K and we see some improvements, doubling up to 93 FPS at 720P. On the internal display we see roughly the same performance.

AYANEO AG01 CYBERPUNK 2077 BENCHMARK COMPARISON

## Game Performance

Benchmarks are useful, but how does the ship handle in actual flight? We continue our AYANEO AG01 review with a look at a cluster of games with the best graphics settings for a minimum of 60 FPS.

### Clair Obscure Expedition 33

The amazing Clair Obscure Expedition 33 runs very well on external displays at 1440P on the Medium graphics settings with some XeSS upscaling. For the internal display you can increase the graphics settings to High.

Clair Obscure Expedition 33 on external monitor

### Forza Horizon 5

Forza Horizon 5 runs great on [AMD](https://droix.net/product-attribute/graphics-gpu-brand/amd/) GPUs, and we can easily set course for 1080P 120Hz on High graphics settings with no FSR needed. You could enable [FSR](https://www.amd.com/en/products/graphics/technologies/fidelityfx/super-resolution.html) and chart a course for even higher graphical fidelity if you wished.

Forza Horizon 5 with the AYANEO AG01

### No Mans Sky

We couldn’t make an AYANEO AG01 review without a space-themed game! In No Man’s Sky, you can explore infinite worlds at 1080P Ultra graphics settings, ensuring great-looking visuals and smooth frame rates as you warp between systems.

Explore new worlds with No Mans Sky on Ultra graphics settings

### Indiana Jones and the Great Circle

For Indiana Jones we are running at 1080P with a mix of Medium and High graphics settings to keep the frame rate well above 60 frames per second.

Or somewhere closer to Earth with Indiana Jones

### The Elder Scrolls IV: Oblivion Remastered

For Oblivion Remastered we are running at 1080P on the High graphics with FSR on Balanced for a solid 60+ FPS.

The Elder Scrolls IV Oblivion Remastered

### Goat Simulator 3

On an external display you can run at 4K on a mix of High and Ultra graphics settings. And of course on the internal display at 1080P you can run on Ultra graphics settings.

Goat Simulator 3 at 4K on the AYANEO AG01

### DOOM Dark Ages

DOOM Dark Ages runs very well. At 1080P you can go right up to the Ultra Nightmare graphics settings with FSR Balanced for 60+ FPS.

DOOM Dark Ages with the AYANEO AG01

## Final Thoughts

Time to sum up our AYANEO AG01 review. The [AYANEO AG01 Starship graphics dock](https://droix.net/product/ayaneo-ag01/) has successfully completed its maiden voyage in our tests and is, overall, a stellar option for an eGPU docking station. We saw excellent performance during our benchmarks, and this continued into real-world gameplay. Besides the performance, the standout feature is, of course, its out-of-this-world design. It makes the AG01 a centerpiece on your desktop, instead of an ominous monolith like the [GPD G1](https://droix.net/product/gpd-g1/) or [ONEXGPU 2](https://droix.net/product/onexplayer-onexgpu-2/). It’s more Starfleet, less Borg cube.

AYANEO AG01 GPD G1 and ONEXPLAYER ONEXGPU 2 compared

As well as the powerful GPU, you also have a multitude of connectivity options with up to quad monitor support, fast ethernet, and SSD storage which is always useful with handheld devices. It effectively transforms your handheld into a full-fledged battle station… or productive desktop setup.

Cyberpunk 2077 runs great on the AYANEO AG01

While there are higher performing ships patrolling the galaxy, the [AYANEO AG01 Starship graphics dock](https://droix.net/product/ayaneo-ag01/) offers a more affordable alternative while remaining a high-performance vessel for its price. It finds a sweet spot in the cosmos of eGPUs, offering a compelling package that’s ready to launch your gaming to new heights.

You can learn more and [buy the AYANEO AG01 Starship Graphics Dock here](https://droix.net/product/ayaneo-ag01/).

[BUY THE AYANEO AG01 STARSHIP HERE](https://droix.net/product/ayaneo-ag01/)

**Captains Log Final Entry:** We hope this report provides the crucial intel you need. If you have any questions about the AYANEO AG01 review or have your own thoughts on this eGPU docking station, please leave them in the comments below.